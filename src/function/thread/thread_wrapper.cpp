...
void Thread::game_thr(map &Map, std::pair<char, char> &from,
                      const std::pair<char, char> &to, int &operation,
                      std::condition_variable &cv, std::mutex &mx) {
  std::unique_lock<std::mutex> uLock(mx, std::defer_lock);
  while (operation != -1) {
    cv.wait(uLock, [&]() { return operation != 0; });
    if (operation == 3) {
      Map.set_update(true);
    } else if (operation == 1) {
      if (Map.move(from, to)) {
...
void Thread::_run() noexcept {
  m_th = std::thread(Thread::game_thr, std::ref(*m_map), std::ref(*m_from),
                     std::ref(*m_to), std::ref(m_operation), std::ref(m_cv),
                     std::ref(m_mx));
}

Thread::Thread(map &Map, std::pair<char, char> &From, std::pair<char, char> &To,
...
void Thread::operation(const OperationType Operation) noexcept {
  ...
  m_cv.notify_one();
  m_cv.wait(uLock, [&]() { return m_operation <= 0; });
}
