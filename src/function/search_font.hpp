...
[[nodiscard]] babel::OPT::optional<std::string> search_font() noexcept
{
    ...
    std::transform(discs.begin(), discs.end(), std::back_inserter(all_files),
    ...
    for ( const auto &WantedFont : wanted_font )
    {
        ...
        {
            auto found = std::find_if(DIRECTORY.second.begin(), DIRECTORY.second.end(),
