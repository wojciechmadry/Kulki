...
void left_click(Thread &thread, sf::RenderWindow &window,
  ...
  if (Resource.get_as<sf::RectangleShape>(ResourceType::MAP_BEFORE_GRID)
          .getGlobalBounds()
    ...
    if (picked.first != -1) // -1 means no picked ball
    {
      ...
      redbox.set_position(
          static_cast<float>(grid_pos.x +
                             side_length * static_cast<float>(picked.second)),
  ...
  } else if (Resource.get_as<sf::RectangleShape>(ResourceType::NEW_GAME_BOX)
                 .getGlobalBounds()
                 .contains(pos)) {
    picked = {-1, -1};
    redbox.hide();
