...
bool map::check_for_score() noexcept {
  ...
  for (auto start : babel::ITERATOR::range<uint8_t>(0, 5)) {
    ...
    for (auto j : babel::ITERATOR::range<uint8_t>(0, 9 - start - 1)) {
      if (!m_grid[start + add_to_i][j].is_empty() &&
        ...
        ++diagonal;
      else if (!m_grid[start + add_to_i + 1][j + 1].is_empty() &&
               diagonal < 5) {
        diagonal_start = {start + add_to_i + 1, j + 1};
        diagonal = 1;
      } else if (diagonal > 4 && m_grid[start + add_to_i][j] !=
                                     m_grid[start + add_to_i + 1][j + 1])
        break;
      ++add_to_i;
    }
    if (diagonal > 4) {
      ...
      m_need_update = true;
      auto color_to_clear =
          m_grid[diagonal_start.first][diagonal_start.second].color();
  ...
  for (auto start : babel::ITERATOR::range<uint8_t>(1, 5)) {
    ...
    for (auto j : babel::ITERATOR::range<uint8_t>(0, 9 - start - 1)) {
      if (!m_grid[j][start + add_to_i].is_empty() &&
        ...
        ++diagonal;
      else if (!m_grid[j + 1][start + add_to_i + 1].is_empty() &&
               diagonal < 5) {
        diagonal_start = {j + 1, start + add_to_i + 1};
        diagonal = 1;
      } else if (diagonal > 4 && m_grid[j][start + add_to_i] !=
                                     m_grid[j + 1][start + add_to_i + 1])
        break;
      ++add_to_i;
    }
    if (diagonal > 4) {
  ...
  for (uint8_t start = 7; start > 3; --start) {
    ...
    for (uint8_t i = 0; i < start; ++i) {
      if (!m_grid[i][start - i].is_empty() &&
          m_grid[i][start - i] == m_grid[i + 1][start - i - 1])
        ++diagonal;
