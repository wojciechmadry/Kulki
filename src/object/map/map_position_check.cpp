...
bool map::is_free_at(
    const std::pair<uint8_t, uint8_t> position) const noexcept {
  return m_grid[position.first][position.second].is_empty();
}

bool map::is_free_at(const uint8_t x, const uint8_t y) const noexcept {
  return m_grid[x][y].is_empty();
}

void map::freed_on(const std::pair<uint8_t, uint8_t> position) noexcept {
  m_grid[position.first][position.second].clear();
}
...
ball &map::at(const std::pair<uint8_t, uint8_t> position) {
  ...
  return m_grid[position.first][position.second];
}
